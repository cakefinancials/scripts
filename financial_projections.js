...
const getHistogram = async ({ samples }) => {
  ...
  await new Promise(resolve => {
    imageStream.pipe(fs.createWriteStream(histogramFile)).on('finish', resolve);
  });
};

const getMeanAndVar = ({ samples }) => {
  const getVariance = (samples, mean) => {
    const sumOfSquares = samples.reduce((pre, sample) => {
  ...
  const getMean = samples => {
    const meanTot = samples.reduce((pre, sample) => {
      return pre + sample;
    }, 0);
