...
const main = async () => {
  console.log('Loading...');
  [stripeSecret, plaidClientId, plaidSecret, plaidPublicKey, plaidEnv, privateKey] = await Promise.all([
  ...
  const privateKeyPassword = readlineSync.question('What is the password for the private key? ', {
    hideEchoBack: true,
  });
  ...
  const processNext = async () => {
    const email = readlineSync.question('What is the email of the user you want to process? ');

    let idLinkKey;
    try {
      idLinkKey = new String(execSync(`aws s3 ls s3://cake-financials-user-data/email_to_cognito_id/${email}/`));
    } catch (e) {
      console.log('could not find a user with that email');
      return;
    }

    const cognitoIdenity = idLinkKey
      .split(' ')
      .pop()
      .trim();
...
const USER_COMMANDS = [
  {
    ...
    fn: async function viewPlaidTransaction({ cognitoIdenity }) {
      const contents = getS3Contents({
        s3Key: `s3://cake-financials-user-data/${cognitoIdenity}/user_plaid_data.json`,
      });
      if (!contents) {
      ...
      try {
        ...
        if (transactions.length === 0) {
          ...
          R.forEach(({ amount, date }) => lines.push(`${date}: ${amount}`), transactions);
        }
      } catch (err) {
  ...
  },
  {
    ...
    fn: async function viewBrokerageCredentials({ cognitoIdenity, privKeyObj }) {
      const brokerageCredentials = getS3Contents({
        s3Key: `s3://cake-financials-user-data/${cognitoIdenity}/brokerage_credentials`,
      });
      const lines = [];
  ...
  },
  {
    ...
    fn: async function viewBrokerageCredentials({ cognitoIdenity }) {
      ...
      const amountToCharge = readlineSync.question('How much do you want to charge? ');

      const parsedAmountToCharge = parseFloat(parseFloat(amountToCharge).toFixed(2));

      const answer = readlineSync.question(
        `Are you sure you want to charge ${parsedAmountToCharge}? Type 'y' for yes: `
      );
...
const getPlaidClient = () => {
  ...
  var plaidClient = new plaid.Client(plaidClientId, plaidSecret, plaidPublicKey, plaid.environments[plaidEnv]);
